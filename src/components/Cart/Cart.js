...
const Cart = () => {
  ...
  const fetchByDateRange = async () => {
    const response = await axios.get(
      `https://fakestoreapi.com/carts?startdate=${selectedDateRange.startDate}&enddate=${selectedDateRange.endDate}`
    );
    getCartData(response?.data);
  };
  ...
  const fetchCartLimit = async (limit) => {
    try {
      const response = await axios.get(
      ...
      const data = await Promise.all(
        response.data.map(async (cart, index) => {
          const productResponse = await axios.get(
            `https://fakestoreapi.com/products/${cart.products.id}`
          );
          return {
            id: cart.id,
            dummyID: index,
            product: productResponse.data, // Access .data to get the response data
      ...
      // Now you can do something with the 'data' array, such as dispatching it or setting state.
    ...
    <div className="container-fluid px-4 px-lg-5 pb-5">
      ...
      <div className="d-flex justify-content-between">
        ...
        <div className="d-flex gap-2">
          ...
          />
          <button onClick={fetchByDateRange} className="btn mt-4 btn-warning ">
            get in Date Range
          </button>
        </div>
      </div>
      ...
      <div className="d-flex w-100 justify-content-center">
        <Pagination data={cart} perPage={3} onPageChange={setFilteredItems} />
      </div>
    </div>
  );
};
